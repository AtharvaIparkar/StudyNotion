  ...
  useEffect(() => {
    ;(async () => {
      ...
      try {
        ...
        setSubLinks(res.data.allCategory || res.data)
      } catch (error) {
...
//   console.log("sub links", subLinks)
  ...
  return (
    ...
    >
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="absolute top-14 left-0 right-0 z-50 bg-richblack-800 border-b border-richblack-700 md:hidden">
          <div className="flex flex-col p-4 space-y-4">
            {/* Mobile Navigation Links */}
            <nav>
              <ul className="flex flex-col space-y-4 text-richblack-25">
                {NavbarLinks.map((link, index) => (
                  <li key={index}>
                    {link.title === "Catalog" ? (
                      <div className="space-y-2">
                        <p className="font-semibold text-richblack-25">{link.title}</p>
                        {loading ? (
            ...
            <div className="flex flex-col space-y-2 pt-4 border-t border-richblack-700">
              {user && user?.accountType !== ACCOUNT_TYPE.INSTRUCTOR && (
                <Link 
                  to="/dashboard/cart" 
                  className="flex items-center space-x-2 text-richblack-25 hover:text-yellow-25"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  ...
                  <Link to="/signup" onClick={() => setMobileMenuOpen(false)}>
                    <button className="w-full rounded-[8px] border border-richblack-700 bg-richblack-800 px-[12px] py-[8px] text-richblack-100">
                      Sign up
                             ...
                             {token !== null && (
                 <div>
                   <ProfileDropdown onNavigate={() => setMobileMenuOpen(false)} />
                 </div>
               )}
            </div>
          </div>
