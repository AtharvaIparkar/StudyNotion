...
export default function Instructor() {
    ...
    useEffect(() => {
      ;(async () => {
        ...
        try {
          ...
          console.log("Courses with students:", result?.map(course => ({
            name: course.courseName,
            students: course.studentsEnrolled?.length || 0,
            studentIds: course.studentsEnrolled || [],
    ...
    const totalStudents = allStudentIds.size || 0
    
    const totalAmount = courses?.reduce(
      (acc, curr) => acc + ((curr.studentsEnrolled?.length || 0) * (curr.price || 0)),
      0
    ) || 0

    console.log("Calculated totals:", { totalAmount, totalStudents, coursesCount: courses?.length || 0 })
